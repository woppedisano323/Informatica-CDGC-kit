...
def create_fs_tables(cur, db):
    ddl = [
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_CORE_SCHEMA}.CUSTOMER_MASTER (
            CUSTOMER_ID VARCHAR(20) NOT NULL PRIMARY KEY, FIRST_NAME VARCHAR(50),
            LAST_NAME VARCHAR(50), DATE_OF_BIRTH DATE, SSN VARCHAR(15),
            EMAIL VARCHAR(100), PHONE_NUMBER VARCHAR(30), ADDRESS_LINE1 VARCHAR(100),
            CITY VARCHAR(50), STATE VARCHAR(2), ZIP_CODE VARCHAR(10),
            ACCOUNT_STATUS VARCHAR(20), CUSTOMER_SINCE DATE, CREDIT_SCORE INTEGER,
            ANNUAL_INCOME FLOAT, RISK_TIER VARCHAR(20))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_CORE_SCHEMA}.TRANSACTION_LEDGER (
            TRANSACTION_ID VARCHAR(20) NOT NULL PRIMARY KEY, ACCOUNT_ID VARCHAR(20),
            CUSTOMER_ID VARCHAR(20), TRANSACTION_DATE TIMESTAMP, TRANSACTION_TYPE VARCHAR(20),
            AMOUNT FLOAT, CURRENCY VARCHAR(5), MERCHANT_NAME VARCHAR(100),
            MERCHANT_CATEGORY VARCHAR(50), BALANCE_AFTER FLOAT, CHANNEL VARCHAR(20),
            STATUS VARCHAR(20), REFERENCE_NUMBER VARCHAR(30))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_CORE_SCHEMA}.GL_ENTRY_REGISTER (
            ENTRY_ID VARCHAR(20) NOT NULL PRIMARY KEY, JOURNAL_ID VARCHAR(20),
            ENTRY_DATE DATE, POSTING_DATE DATE, ACCOUNT_CODE VARCHAR(10),
            ACCOUNT_NAME VARCHAR(100), DEBIT_AMOUNT FLOAT, CREDIT_AMOUNT FLOAT,
            NET_AMOUNT FLOAT, COST_CENTER VARCHAR(50), LEGAL_ENTITY VARCHAR(100),
            CURRENCY VARCHAR(5), FISCAL_PERIOD VARCHAR(5), FISCAL_YEAR INTEGER,
            CREATED_BY VARCHAR(100), APPROVED_BY VARCHAR(100))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_CORE_SCHEMA}.RISK_EXPOSURE_DAILY (
            RISK_ID VARCHAR(20) NOT NULL PRIMARY KEY, CUSTOMER_ID VARCHAR(20),
            ASSESSMENT_DATE DATE, RISK_CATEGORY VARCHAR(30), EXPOSURE_AMOUNT FLOAT,
            PROBABILITY_OF_DEFAULT FLOAT, LOSS_GIVEN_DEFAULT FLOAT, EXPECTED_LOSS FLOAT,
            RISK_RATING VARCHAR(5), COLLATERAL_VALUE FLOAT, REGULATORY_CAPITAL FLOAT,
            BASEL_CLASS VARCHAR(30))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_TRAINING_SCHEMA}.TRANSACTION_FEATURES (
            TRANSACTION_ID VARCHAR(36), ACCOUNT_ID VARCHAR(20),
            TRANSACTION_AMOUNT FLOAT, MERCHANT_CATEGORY_CODE VARCHAR(5),
            MERCHANT_NAME VARCHAR(100), TRANSACTION_TIMESTAMP TIMESTAMP,
            DEVICE_FINGERPRINT VARCHAR(50), CHANNEL VARCHAR(20),
            GEOLOCATION_COUNTRY VARCHAR(5), VELOCITY_1H INTEGER, VELOCITY_24H INTEGER,
            AVG_TRANSACTION_30D FLOAT, IS_INTERNATIONAL BOOLEAN, FRAUD_LABEL INTEGER,
            DATA_PARTITION VARCHAR(10), RECORD_CREATED_DATE DATE)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_TRAINING_SCHEMA}.CUSTOMER_360_FEATURES (
            CUSTOMER_ID VARCHAR(20), AGE INTEGER, ANNUAL_INCOME FLOAT,
            EMPLOYMENT_STATUS VARCHAR(20), CREDIT_SCORE INTEGER,
            DEBT_TO_INCOME_RATIO FLOAT, EXISTING_LOAN_COUNT INTEGER,
            MONTHS_CUSTOMER INTEGER, KYC_DOCUMENT_TYPE VARCHAR(30),
            KYC_DOCUMENT_COUNTRY VARCHAR(5), KYC_VERIFIED_DATE DATE,
            KYC_EXPIRY_DATE DATE, PEP_FLAG BOOLEAN, SANCTIONS_FLAG BOOLEAN,
            DEFAULT_LABEL INTEGER, DATA_PARTITION VARCHAR(10), RECORD_CREATED_DATE DATE)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_TRAINING_SCHEMA}.RISK_EXPOSURE_FEATURES (
            EXPOSURE_ID VARCHAR(36), COUNTERPARTY_ID VARCHAR(20), ASSET_CLASS VARCHAR(30),
            OUTSTANDING_BALANCE FLOAT, ORIGINAL_BALANCE FLOAT, MATURITY_DATE DATE,
            INTEREST_RATE FLOAT, COLLATERAL_VALUE FLOAT, LTV_RATIO FLOAT,
            DAYS_PAST_DUE INTEGER, INTERNAL_RATING VARCHAR(5), STRESS_SCENARIO VARCHAR(20),
            PROJECTED_LOSS FLOAT, REGULATORY_CAPITAL FLOAT, RECORD_QUARTER VARCHAR(10),
            RECORD_CREATED_DATE DATE)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_TRAINING_SCHEMA}.REGULATORY_SUBMISSIONS_FEATURES (
            SUBMISSION_ID VARCHAR(36), FILING_TYPE VARCHAR(20), REGULATORY_BODY VARCHAR(10),
            REPORTING_PERIOD VARCHAR(10), SUBMISSION_DATE DATE, DEADLINE_DATE DATE,
            FILED_ON_TIME BOOLEAN, RESTATEMENT_COUNT INTEGER, ERROR_COUNT INTEGER,
            COMPLETENESS_SCORE FLOAT, ACCEPTANCE_STATUS VARCHAR(20),
            REJECTION_REASON VARCHAR(200), VALIDATION_LABEL INTEGER,
            DATA_PARTITION VARCHAR(10), RECORD_CREATED_DATE DATE)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_MODEL_SCHEMA}.FRAUD_DETECTOR_IO (
            TRANSACTION_AMOUNT FLOAT, MERCHANT_CATEGORY_CODE VARCHAR(5),
            DEVICE_FINGERPRINT VARCHAR(50), VELOCITY_1H INTEGER, VELOCITY_24H INTEGER,
            AVG_TRANSACTION_30D FLOAT, IS_INTERNATIONAL BOOLEAN,
            GEOLOCATION_COUNTRY VARCHAR(5), FRAUD_PROBABILITY FLOAT,
            DECISION VARCHAR(20), CONFIDENCE_SCORE FLOAT, MODEL_VERSION VARCHAR(10),
            INFERENCE_TIMESTAMP TIMESTAMP)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_MODEL_SCHEMA}.BEHAVIORAL_ANOMALY_IO (
            ACCOUNT_ID VARCHAR(20), VELOCITY_24H INTEGER, AVG_TRANSACTION_30D FLOAT,
            CHANNEL VARCHAR(20), DEVICE_FINGERPRINT VARCHAR(50),
            TRANSACTION_AMOUNT FLOAT, GEOLOCATION_COUNTRY VARCHAR(5),
            ANOMALY_SCORE FLOAT, ANOMALY_TYPE VARCHAR(30), RISK_TIER VARCHAR(10),
            MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_MODEL_SCHEMA}.AML_ANOMALY_IO (
            ACCOUNT_ID VARCHAR(20), TRANSACTION_AMOUNT FLOAT,
            MERCHANT_CATEGORY_CODE VARCHAR(5), VELOCITY_24H INTEGER,
            IS_INTERNATIONAL BOOLEAN, GEOLOCATION_COUNTRY VARCHAR(5),
            SANCTIONS_FLAG BOOLEAN, PEP_FLAG BOOLEAN, SAR_PROBABILITY FLOAT,
            AML_RISK_SCORE FLOAT, ALERT_LEVEL VARCHAR(10),
            MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_MODEL_SCHEMA}.CREDIT_RISK_IO (
            CUSTOMER_ID VARCHAR(20), ANNUAL_INCOME FLOAT, CREDIT_SCORE INTEGER,
            DEBT_TO_INCOME_RATIO FLOAT, EMPLOYMENT_STATUS VARCHAR(20),
            EXISTING_LOAN_COUNT INTEGER, MONTHS_CUSTOMER INTEGER,
            OUTSTANDING_BALANCE FLOAT, PD_SCORE FLOAT, RISK_GRADE VARCHAR(5),
            APPROVED_LIMIT FLOAT, DECISION VARCHAR(20),
            MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_MODEL_SCHEMA}.KYC_CLASSIFIER_IO (
            CUSTOMER_ID VARCHAR(20), KYC_DOCUMENT_TYPE VARCHAR(30),
            KYC_DOCUMENT_COUNTRY VARCHAR(5), PEP_FLAG BOOLEAN, SANCTIONS_FLAG BOOLEAN,
            KYC_EXPIRY_DATE DATE, VERIFICATION_STATUS VARCHAR(20),
            RISK_CLASSIFICATION VARCHAR(20), CONFIDENCE_SCORE FLOAT,
            MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_MODEL_SCHEMA}.REGULATORY_VALIDATOR_IO (
            SUBMISSION_ID VARCHAR(36), FILING_TYPE VARCHAR(20),
            COMPLETENESS_SCORE FLOAT, ERROR_COUNT INTEGER, RESTATEMENT_COUNT INTEGER,
            DAYS_BEFORE_DEADLINE INTEGER, PASS_PROBABILITY FLOAT,
            PREDICTED_STATUS VARCHAR(20), REJECTION_RISK_FACTORS VARCHAR(200),
            MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{FS_MODEL_SCHEMA}.CCAR_STRESS_IO (
            EXPOSURE_ID VARCHAR(36), ASSET_CLASS VARCHAR(30),
            OUTSTANDING_BALANCE FLOAT, LTV_RATIO FLOAT, INTEREST_RATE FLOAT,
            DAYS_PAST_DUE INTEGER, INTERNAL_RATING VARCHAR(5), STRESS_SCENARIO VARCHAR(20),
            PROJECTED_LOSS FLOAT, LOSS_RATE FLOAT, REGULATORY_CAPITAL FLOAT,
            CET1_IMPACT FLOAT, MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
    ]
    for stmt in ddl:
        _exec(cur, stmt)
...
def create_hc_tables(cur, db):
    ddl = [
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_CORE_SCHEMA}.PATIENT_DEMOGRAPHICS (
            PATIENT_ID VARCHAR(20) NOT NULL PRIMARY KEY, FIRST_NAME VARCHAR(50),
            LAST_NAME VARCHAR(50), DATE_OF_BIRTH DATE, SEX VARCHAR(10),
            RACE VARCHAR(50), ETHNICITY VARCHAR(50), PREFERRED_LANGUAGE VARCHAR(50),
            ADDRESS_LINE1 VARCHAR(100), CITY VARCHAR(50), STATE VARCHAR(2),
            ZIP_CODE VARCHAR(10), PHONE_NUMBER VARCHAR(20), EMAIL VARCHAR(100),
            EMERGENCY_CONTACT VARCHAR(100), CONSENT_ON_FILE BOOLEAN,
            CONSENT_DATE DATE, PATIENT_SINCE DATE,
            INSURANCE_PLAN_ID VARCHAR(20), PCP_PROVIDER_ID VARCHAR(20))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_CORE_SCHEMA}.CLINICAL_ENCOUNTERS (
            ENCOUNTER_ID VARCHAR(20) NOT NULL PRIMARY KEY, PATIENT_ID VARCHAR(20),
            ENCOUNTER_DATE TIMESTAMP, ENCOUNTER_TYPE VARCHAR(30), FACILITY_CODE VARCHAR(20),
            PROVIDER_ID VARCHAR(20), ICD10_CODE VARCHAR(10), PROCEDURE_CODE VARCHAR(10),
            DRG_CODE VARCHAR(10), ADMISSION_DATE DATE, DISCHARGE_DATE DATE,
            DISCHARGE_DISPOSITION VARCHAR(30), ADMISSION_TYPE VARCHAR(20),
            LENGTH_OF_STAY INTEGER, STATUS VARCHAR(20), PAYER_TYPE VARCHAR(30))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_CORE_SCHEMA}.LAB_RESULTS (
            RESULT_ID VARCHAR(20) NOT NULL PRIMARY KEY, PATIENT_ID VARCHAR(20),
            ENCOUNTER_ID VARCHAR(20), LAB_TEST_NAME VARCHAR(100), LOINC_CODE VARCHAR(10),
            LAB_RESULT_VALUE VARCHAR(50), NUMERIC_VALUE FLOAT, LAB_RESULT_UNITS VARCHAR(20),
            LAB_REFERENCE_RANGE VARCHAR(50), ABNORMAL_FLAG VARCHAR(10),
            RESULT_DATE TIMESTAMP, ORDERING_PROVIDER VARCHAR(20),
            SPECIMEN_TYPE VARCHAR(30), STATUS VARCHAR(20))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_CORE_SCHEMA}.MEDICATION_ORDERS (
            ORDER_ID VARCHAR(20) NOT NULL PRIMARY KEY, PATIENT_ID VARCHAR(20),
            ENCOUNTER_ID VARCHAR(20), MEDICATION_NAME VARCHAR(100), NDC_CODE VARCHAR(15),
            RXNORM_CODE VARCHAR(10), MEDICATION_DOSE FLOAT, DOSE_UNIT VARCHAR(20),
            ROUTE_OF_ADMINISTRATION VARCHAR(30), FREQUENCY VARCHAR(30),
            START_DATE DATE, END_DATE DATE, PRESCRIBER_ID VARCHAR(20),
            MEDICATION_STATUS VARCHAR(20), DISPENSE_QUANTITY FLOAT,
            REFILLS_AUTHORIZED INTEGER)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_CORE_SCHEMA}.IMMUNIZATION_RECORDS (
            IMMUNIZATION_ID VARCHAR(20) NOT NULL PRIMARY KEY, PATIENT_ID VARCHAR(20),
            IMMUNIZATION_NAME VARCHAR(100), CVX_CODE VARCHAR(5),
            IMMUNIZATION_DATE DATE, LOT_NUMBER VARCHAR(20), MANUFACTURER VARCHAR(50),
            ROUTE VARCHAR(20), SITE VARCHAR(20), ADMINISTERED_BY VARCHAR(20),
            DOSE_NUMBER INTEGER, SERIES_COMPLETE BOOLEAN, REGISTRY_REPORTED BOOLEAN)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_CORE_SCHEMA}.SDOH_ASSESSMENTS (
            ASSESSMENT_ID VARCHAR(20) NOT NULL PRIMARY KEY, PATIENT_ID VARCHAR(20),
            ENCOUNTER_ID VARCHAR(20), ASSESSMENT_DATE DATE, SCREENING_TOOL VARCHAR(50),
            HOUSING_INSTABILITY BOOLEAN, FOOD_INSECURITY BOOLEAN,
            TRANSPORTATION_BARRIER BOOLEAN, INTERPERSONAL_VIOLENCE BOOLEAN,
            FINANCIAL_STRAIN BOOLEAN, SOCIAL_ISOLATION BOOLEAN,
            SDOH_RISK_SCORE INTEGER, REFERRAL_MADE BOOLEAN,
            REFERRAL_TYPE VARCHAR(100), ADMINISTERED_BY VARCHAR(20))""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_CORE_SCHEMA}.PHI_DISCLOSURE_LOG (
            DISCLOSURE_ID VARCHAR(20) NOT NULL PRIMARY KEY, PATIENT_ID VARCHAR(20),
            DISCLOSURE_DATE TIMESTAMP, DISCLOSURE_TYPE VARCHAR(30),
            RECIPIENT_NAME VARCHAR(100), RECIPIENT_TYPE VARCHAR(30),
            PHI_CATEGORY VARCHAR(50), MINIMUM_NECESSARY BOOLEAN,
            AUTHORIZATION_TYPE VARCHAR(30), AUTHORIZATION_ID VARCHAR(20),
            PURPOSE_OF_DISCLOSURE VARCHAR(200), DATA_ELEMENTS_SHARED VARCHAR(500),
            DISCLOSED_BY VARCHAR(20), HIPAA_EXCEPTION_APPLIED BOOLEAN)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_TRAINING_SCHEMA}.SEPSIS_RISK_FEATURES (
            PATIENT_ID VARCHAR(20), ENCOUNTER_ID VARCHAR(20),
            HEART_RATE FLOAT, RESPIRATORY_RATE FLOAT, TEMPERATURE_C FLOAT,
            SYSTOLIC_BP FLOAT, DIASTOLIC_BP FLOAT, SPO2 FLOAT,
            WBC_COUNT FLOAT, LACTATE_LEVEL FLOAT, CREATININE FLOAT,
            GLASGOW_COMA_SCORE INTEGER, VASOPRESSOR_USE BOOLEAN,
            MECHANICAL_VENTILATION BOOLEAN, SOFA_SCORE INTEGER,
            PRIOR_SEPSIS BOOLEAN, ICU_HOURS INTEGER, AGE_AT_ENCOUNTER INTEGER,
            SEPSIS_LABEL INTEGER, DATA_PARTITION VARCHAR(10),
            RECORD_CREATED_DATE DATE)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_TRAINING_SCHEMA}.NLP_DOCUMENT_CORPUS (
            DOCUMENT_ID VARCHAR(36), PATIENT_ID VARCHAR(20), ENCOUNTER_ID VARCHAR(20),
            DOCUMENT_TYPE VARCHAR(30), DOCUMENT_DATE DATE, AUTHOR_SPECIALTY VARCHAR(50),
            NOTE_TEXT VARCHAR(5000), WORD_COUNT INTEGER, ICD10_MENTIONS VARCHAR(500),
            LOINC_MENTIONS VARCHAR(500), MEDICATION_MENTIONS VARCHAR(500),
            SENTIMENT_LABEL VARCHAR(20), NLP_EXTRACT_LABEL INTEGER,
            PHI_SCRUBBED BOOLEAN, DATA_PARTITION VARCHAR(10),
            RECORD_CREATED_DATE DATE)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_MODEL_SCHEMA}.SEPSIS_RISK_IO (
            PATIENT_ID VARCHAR(20), ENCOUNTER_ID VARCHAR(20),
            HEART_RATE FLOAT, RESPIRATORY_RATE FLOAT, TEMPERATURE_C FLOAT,
            SYSTOLIC_BP FLOAT, WBC_COUNT FLOAT, LACTATE_LEVEL FLOAT,
            SOFA_SCORE INTEGER, SEPSIS_RISK_SCORE FLOAT, RISK_TIER VARCHAR(10),
            ALERT_TRIGGERED BOOLEAN, RECOMMENDED_ACTION VARCHAR(100),
            MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
        f"""CREATE TABLE IF NOT EXISTS {db}.{HC_MODEL_SCHEMA}.CLINICAL_NLP_IO (
            DOCUMENT_ID VARCHAR(36), PATIENT_ID VARCHAR(20), DOCUMENT_TYPE VARCHAR(30),
            NOTE_TEXT VARCHAR(5000), EXTRACTED_ICD10 VARCHAR(500),
            EXTRACTED_MEDICATIONS VARCHAR(500), EXTRACTED_LOINC VARCHAR(500),
            CLINICAL_FINDINGS VARCHAR(1000), PHI_DETECTED BOOLEAN,
            ACTIONABILITY_SCORE FLOAT, CONFIDENCE_SCORE FLOAT,
            MODEL_VERSION VARCHAR(10), INFERENCE_TIMESTAMP TIMESTAMP)""",
    ]
    for stmt in ddl:
        _exec(cur, stmt)
